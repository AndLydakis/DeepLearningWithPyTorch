...
def getCandidateInfoList(requireOnDisk_bool=True):
    ...
    s = [os.path.split(p)[-1][:-4] for p in mhd_list]
    print(s)
        ...
        for candidateInfo_tup in positiveInfo_list:
            ...
            try:
                while self.hu_a[ci, cr + row_radius, cc] > threshold_hu and self.hu_a[
                    ci - row_radius, cr, cc] > threshold_hu:
                    row_radius += 1
            except IndexError:
                row_radius -= 1
            ...
            ci - row_radius: ci + row_radius + 1,
            ci - col_radius: ci + col_radius + 1
            ] = True
...
def getCtRawCandidate(series_uid, center_xyz, width_irc):
    ct = getCt(series_uid)
    ct_chunk, pos_chunk_irc, center_irc = ct.getRawCandidate(center_xyz, width_irc)
    return ct_chunk, pos_chunk_irc, center_irc
